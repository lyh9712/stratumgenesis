...
> **历史阶段说明**：本文档描述 v0.2 最小区块层原型（阶段 2）的五阶段校验流水线，
> 属历史归档。当前 v0.3 阶段 C 的校验流水线已扩展为 **9 个检查阶段**：
> 签名 → 结构 → 创世内核兼容 → PoI → 解析 → 特性激活 → 沙箱 → 语言正负测试 → UTXO。
> 现行阶段顺序与正负测试原理见 `EVOLUTION_IMPLEMENTATION.md`（v0.3 阶段 C）。
...
Block dataclass + canonical JSON + SHA-256
        ↓
BlockValidator（五阶段顺序校验，v0.2 历史阶段）
        ↓
ChainStore（主链 / 休眠分支）
```

校验顺序严格为（v0.2 历史阶段的五阶段；当前 9 检查见文首标注）：
